...
app.get("/api/ai/openai-prompt", async (req, res) => {
    ...
    if (!prompt || !msg) return res.json("Masukan Query Parameters");
...
app.get("/api/ai/openai", async (req, res) => {
    const { msg } = req.query;
    if (!msg) return res.json("Masukan Query Parameters");
...
app.get("/api/ai/gpt4", async (req, res) => {
    ...
    if (!text) return res.json("Masukan Query Parameters");
...
app.get("/api/tools/remini", async (req, res) => {
    try {     
      const { url } = req.query
      if (!url) return res.json("Masukan Query Parameters");
...
app.get("/api/tools/tiktokstalk", async (req, res) => {
    try {     
      ...
      if (!user) return res.json("Masukan Query Parameters");
...
if (!egg || !nestid || !loc || !domain || !ptla || !ptlc) return res.json("Masukan Query Parameters")
domain = "https://" + domain
let listnya = []
...
app.get("/api/imagecreator/pornhub", async (req, res) => {
    try {     
      ...
      if (!text1 || !text2) return res.json("Masukan Query Parameters");
...
app.get("/api/imagecreator/qc", async (req, res) => {
    try {     
      ...
      if (!text || !nama || !fotoUrl) return res.json("Masukan Query Parameters");
const json = {
  "type": "quote",
  "format": "png",
